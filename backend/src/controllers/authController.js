...
// Helper to remove sensitive data before sending user to client
...
const register = asyncHandler(async (req, res) => {
  ...
  // If user exists but unverified, update their details. Otherwise, insert new user.
...
const verifyRegisterOtp = asyncHandler(async (req, res) => {
  ...
  const token = signToken(user); // Log them in automatically
...
const login = asyncHandler(async (req, res) => {
  ...
  // If they never verified their email, send another OTP
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // To prevent email enumeration, always say "If this email exists..."
  if (result.rows.length === 0) {
